...
async function verifyStripeSignature(admin: any, raw: string, signature: string) {
  const timestamp = signature.match(/(?:^|,)t=([^,]+)/)?.[1];
  const signatures = [...signature.matchAll(/(?:^|,)v1=([^,]+)/g)].map((m) => m[1]);
  if (!timestamp || signatures.length === 0 || Math.abs(Date.now() / 1000 - Number(timestamp)) > 300) return false;

  const secretNames = ["luxe_stripe_webhook_account", "luxe_stripe_webhook_connect"];
  for (const secretName of secretNames) {
    const { data: secret, error } = await admin.rpc("luxe_get_runtime_secret", { secret_name: secretName });
    if (error || typeof secret !== "string" || !secret) continue;
    ...
    if (signatures.some((value) => secureEqual(value, expected))) return true;
  }
  return false;
}
...
Deno.serve(async (req) => {
  ...
  if (!stripeKey) return new Response("Stripe credentials not configured", { status: 503 });

  const admin = createClient(Deno.env.get("SUPABASE_URL")!, Deno.env.get("SUPABASE_SERVICE_ROLE_KEY")!, { auth: { persistSession: false, autoRefreshToken: false } });
  const raw = await req.text();
  const signature = req.headers.get("stripe-signature") || "";
  if (!(await verifyStripeSignature(admin, raw, signature))) return new Response("Invalid signature", { status: 400 });
  ...
  const inserted = await admin.from("cs_payment_events").insert({
  ...
  if (event.type === "payment_intent.succeeded" && paymentIntentId) {
    ...
    if (payment && !payment.stripe_transfer_id && booking?.status === "completed" && stylist?.stripe_account_id && stylist.stripe_payouts_enabled) {
      const transferBody = new URLSearchParams({
        amount: String(payment.stylist_payout_cents),
        currency: "usd",
        destination: stylist.stripe_account_id,
        transfer_group: `luxe_booking_${payment.booking_id}`,
        "metadata[brand]": "LUXE_ON_DEMAND",
        "metadata[luxe_booking_id]": payment.booking_id,
      });
      const transferResponse = await fetch("https://api.stripe.com/v1/transfers", {
        method: "POST",
        headers: { authorization: `Bearer ${stripeKey}`, "content-type": "application/x-www-form-urlencoded", "Idempotency-Key": `luxe_release_${payment.booking_id}` },
        body: transferBody,
      });
      const transfer = await transferResponse.json();
      if (transferResponse.ok) {
        await admin.from("cs_booking_payments").update({ status: "released", stripe_transfer_id: transfer.id, released_at: new Date().toISOString(), updated_at: new Date().toISOString() }).eq("id", payment.id);
      }
    }
  }

  if (event.type === "account.updated" && object.id) {
    await admin.from("cs_stylists").update({
      stripe_charges_enabled: !!object.charges_enabled,
      stripe_payouts_enabled: !!object.payouts_enabled,
      stripe_details_submitted: !!object.details_submitted,
      stripe_onboarding_complete: !!object.details_submitted && !!object.payouts_enabled,
