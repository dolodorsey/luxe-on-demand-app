...
const APP_ORIGIN = Deno.env.get("LUXE_APP_ORIGIN") || "https://luxe-on-demand-app.vercel.app";
const headers = {
...
Deno.serve(async(req)=>{
  ...
  try{
    ...
    if(action==="connect_onboarding"){
      const {data:stylist}=await admin.from("cs_stylists")
        .select("id,stripe_account_id,license_verified,id_verified,bg_check_passed,insurance_verified")
        .eq("user_id",profile.id).single();
      const verified=stylist&&stylist.license_verified===true&&stylist.id_verified===true&&stylist.bg_check_passed===true&&stylist.insurance_verified===true;
      if(!verified)return json({error:"Fully verified stylist profile required"},403);
      let accountId=stylist.stripe_account_id;
      if(!accountId){
        const account=await stripe("accounts",new URLSearchParams({type:"express",country:"US","capabilities[transfers][requested]":"true","metadata[luxe_stylist_id]":stylist.id}));
        accountId=account.id;
        await admin.from("cs_stylists").update({stripe_account_id:accountId}).eq("id",stylist.id);
      }
      const link=await stripe("account_links",new URLSearchParams({account:accountId,refresh_url:`${APP_ORIGIN}?connect=refresh`,return_url:`${APP_ORIGIN}?connect=complete`,type:"account_onboarding"}));
    ...
    const {data:stylist}=booking.stylist_id?await admin.from("cs_stylists").select("id,user_id,stripe_account_id,stripe_payouts_enabled").eq("id",booking.stylist_id).single():{data:null};
    ...
    if(action==="authorize"){
      ...
      const fee=Math.round(amount*.2),payout=amount-fee;
      let customerId=profile.stripe_customer_id;
      if(!customerId){
        const customer=await stripe("customers",new URLSearchParams({email:auth.user.email||"","metadata[luxe_user_id]":profile.id}));
      ...
      const intent=await stripe("payment_intents",new URLSearchParams({amount:String(amount),currency:"usd",customer:customerId,capture_method:"manual","automatic_payment_methods[enabled]":"true",transfer_group:`luxe_booking_${booking.id}`,"metadata[luxe_booking_id]":booking.id,"metadata[luxe_stylist_id]":stylist.id}));
