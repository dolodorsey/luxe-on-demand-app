...
Deno.serve(async (req) => {
  ...
  const stripeSecret = (await readSecret('LUXE_MOBILITY_STRIPE_SECRET_KEY')) || (await readSecret('STRIPE_SECRET_KEY'))
  ...
  if (action === 'capture') {
    ...
    const { data: ride } = await admin.from('lm_rides').select('id,driver_id,status').eq('id', rideId).maybeSingle()
    ...
    await admin.from('lm_payments').update({ status: 'capture_pending', driver_id: driver.id, updated_at: new Date().toISOString() }).eq('id', payment.id)
    try {
      ...
      await admin.from('lm_payments').update({
        status: intent.status === 'succeeded' ? 'captured' : 'capture_pending',
      ...
      return json({
        ...
        settlementReady: feeBps !== null && feeBps !== undefined,
        settlementReason: feeBps === null || feeBps === undefined ? 'platform_fee_unconfigured' : 'webhook_reconciliation',
      })
    } catch (error) {
      await admin.from('lm_payments').update({ status: 'authorized', failure_message: error instanceof Error ? error.message : 'Capture failed', updated_at: new Date().toISOString() }).eq('id', payment.id)
      return json({ error: 'Fare capture failed; authorization remains held for retry.' }, 502)
    }
  }
