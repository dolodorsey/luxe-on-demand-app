...
Deno.serve(async (req) => {
  ...
  if (!url || !anon || !serviceRole) return json({ error: 'LUXE payout onboarding is not configured.' }, 503)
  ...
  if (!stripeSecret) return json({ error: 'LUXE payout onboarding is not configured.' }, 503)
  ...
  const link = await stripe.accountLinks.create({
    ...
    refresh_url: `${origin}/driver?connect=refresh`,
    return_url: `${origin}/driver?connect=return`,
  })
